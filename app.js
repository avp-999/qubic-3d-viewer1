...
const ui = {
  ...
  branch: $('#branch-select'), timeline: $('#timeline'), timelineValue: $('#timeline-value'), tree: $('#tree-view'),
  graphTree: $('#decision-tree-canvas'),
...
function boardMoves() {
  return path.slice(1).map((id) => {
    ...
    // El movimiento almacenado en el hijo lo realiza el jugador cuyo turno
    // estaba indicado por el nodo padre.
    const inferred = previous?.player === 'X' || previous?.player === 'O' ? previous.player : node.player;
    return { move: node.move, player: inferred };
  }).filter((item) => Array.isArray(item.move));
}
function legalBoardForPath() {
  const cells = new Map();
  boardMoves().forEach((item) => cells.set(item.move.join(','), item.player));
  return cells;
}
function ensureDynamicChildren(node, moves) {
  if (!graph || node.children.length || node.is_terminal || moves.length >= MAX_GAME_PLIES) return;
  const cells = legalBoardForPath();
  const player = node.player === 'X' ? 'X' : 'O';
  const nextPlayer = player === 'X' ? 'O' : 'X';
  const children = [];
  for (let x = 0; x < 3; x++) for (let z = 0; z < 3; z++) {
    let y = 0;
    while (y < 3 && cells.has(`${x},${y},${z}`)) y++;
    if (y >= 3) continue;
    const move = [x, y, z];
    const nextMoves = [...moves, { move, player }];
    const winning = findWinningCombos(nextMoves);
    const id = `lazy-${node.id}-${x}${y}${z}`;
    graph.nodes[id] = { id, move, player: nextPlayer, is_terminal: winning.length > 0 || nextMoves.length >= 27, winner: winning.length ? player : null, status: 'dynamic', children: [] };
    children.push(id);
  }
...
function findPathTo(targetId, nodeId = graph.rootId, visited = new Set()) {
  if (nodeId === targetId) return [nodeId];
  if (visited.has(nodeId)) return null;
  visited.add(nodeId);
  for (const childId of getNode(nodeId).children) {
    const childPath = findPathTo(targetId, childId, visited);
...
function renderDecisionTree() {
  ...
  const buildList = (nodeId, seen = new Set()) => {
    const list = document.createElement('ul');
    list.className = 'tree-list';
    if (rendered >= renderLimit) {
      const more = document.createElement('li');
      more.className = 'tree-node-button';
      more.textContent = '… más ramas cargadas en el DAG';
      list.appendChild(more);
      return list;
    }
    rendered += 1;
    if (seen.has(nodeId)) return list;
    ...
    if (nodeId === activeId) button.classList.add('is-active');
    if (pathIds.has(nodeId)) button.classList.add('is-path');
    if (node.is_terminal) button.classList.add('is-terminal');
    const moveText = node.move ? ` · ${node.move.join(',')}` : ' · inicio';
    button.textContent = `${nodeId} · ${node.player}${moveText}${node.is_terminal ? ' · ✓' : ''}`;
    button.title = `Ir al nodo ${nodeId}`;
    button.addEventListener('click', () => {
      const targetPath = findPathTo(nodeId);
      if (targetPath) { path = targetPath; renderNode(); }
    });
    item.appendChild(button);
    list.appendChild(item);
    if (node.children.length) {
      const childList = document.createElement('div');
      node.children.forEach((childId) => childList.appendChild(buildList(childId, nextSeen)));
      item.appendChild(childList);
    }
    return list;
  };
  ui.tree.appendChild(buildList(graph.rootId));
}
function renderGraphTree() {
  ui.graphTree.replaceChildren();
  if (!graph) return;
  const activeId = path.at(-1);
  const expanded = new Set(path);
  const makeNode = (nodeId, seen = new Set()) => {
    const node = getNode(nodeId);
    const li = document.createElement('li');
    const button = document.createElement('button');
    button.type = 'button';
    button.className = `graph-node${nodeId === activeId ? ' active' : ''}${node.is_terminal ? ' terminal' : ''}`;
    button.textContent = `${nodeId} · ${node.player}`;
    button.title = node.move ? `Jugada: [${node.move.join(', ')}]` : 'Posición inicial';
    button.addEventListener('click', () => {
      const targetPath = findPathTo(nodeId);
      if (targetPath) { path = targetPath; renderNode(); }
    });
    li.appendChild(button);
    if (node.children.length && expanded.has(nodeId) && !seen.has(nodeId)) {
      const ul = document.createElement('ul');
      const nextSeen = new Set(seen).add(nodeId);
      node.children.forEach((childId) => ul.appendChild(makeNode(childId, nextSeen)));
      li.appendChild(ul);
    }
    return li;
  };
  ...
  root.appendChild(makeNode(graph.rootId));
  ui.graphTree.appendChild(root);
}
function renderNode() {
  currentNode = getNode(path[path.length - 1]);
  const moves = boardMoves();
  const winning = findWinningCombos(moves);
  ensureDynamicChildren(currentNode, moves);
