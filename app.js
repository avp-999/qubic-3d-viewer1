...
const ui = {
  ...
  graphTree: $('#decision-tree-canvas'), gravity: $('#gravity-toggle'),
...
function boardMoves() {
  const rawMoves = path.slice(1).map((id) => {
  ...
  if (!ui.gravity.checked) return rawMoves;
  const heights = new Map();
  return rawMoves.map((item) => {
    const [x, , z] = item.move;
    const key = `${x},${z}`;
    const y = heights.get(key) || 0;
    heights.set(key, y + 1);
    return { ...item, move: [x, Math.min(2, y), z] };
  });
}
...
function renderGraphTree() {
  ...
  const makeNode = (nodeId, seen = new Set()) => {
    ...
    if (node.children.length && !seen.has(nodeId)) {
...
ui.gravity.addEventListener('change', renderNode);
